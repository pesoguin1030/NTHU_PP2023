...
__global__ void sobel_cuda(unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels, int (*mask)[MASK_X][MASK_Y]) {
        ...
        for (int v = -yBound; v < yBound + adjustY; ++v) {
            for (int u = -xBound; u < xBound + adjustX; ++u) {
                if ((x + u) >= 0 && (x + u) < width && (y + v) >= 0 && (y + v) < height) {
                    int R = s[channels * (width * (y + v) + (x + u)) + 2];
                    int G = s[channels * (width * (y + v) + (x + u)) + 1];
                    int B = s[channels * (width * (y + v) + (x + u)) + 0];
...
int main(int argc, char** argv) {
    ...
    cudaMalloc(&device_mask, MASK_N * MASK_X * MASK_Y * sizeof(int));
    ...
    cudaMemcpy(device_mask, mask, MASK_N * MASK_X * MASK_Y * sizeof(int), cudaMemcpyHostToDevice);

    /* Hint 3 */
    // acclerate this function
    dim3 dimBlock(16, 16); // Block size (adjust as needed)
    dim3 dimGrid((width + dimBlock.x - 1) / dimBlock.x, (height + dimBlock.y - 1) / dimBlock.y);
    sobel_cuda<<<dimGrid, dimBlock>>>(device_s, device_t, height, width, channels, device_mask);
