...
void Sigmoid(float *A, int n, int m) {
    //#pragma acc data copyin(A[0:n*m]) copyout(A[0:m*n])
...
void Argmax(float *A, int *D, int n, int m) {
    ...
    for (int i = 0; i < n; i++) {
        float mx = A[i * m];
        int index = 0;
        #pragma acc loop reduction(max:mx) // To handle the max value in parallel
        for (int j = 1; j < m; j++) {
            if (mx < A[i * m + j]) {
                mx = A[i * m + j];
                index = j;
    ...
    #pragma acc exit data copyout(A[0:n*m])
}
...
//             #pragma acc loop reduction(max:mx)
//             for (int j = 1; j < m; j++) {
//                 if (mx < A[i * m + j]) {
//                     mx = A[i * m + j];
//                     //index = j;
...
//     // 将输入数组 A 复制到 GPU，并将 max_values 数组也复制到 GPU
//     #pragma acc data copyin(A[0:n*m]) copy(max_values[0:n])
//     {
//         #pragma acc parallel loop
//         for (int i = 0; i < n; i++) {
//             float mx = A[i * m];
...
//     // 使用 OpenACC 并行寻找每个最大值的索引
//     #pragma acc data copyin(A[0:n*m], max_values[0:n]) copyout(D[0:n])
//     {
//         #pragma acc parallel loop
//         for (int i = 0; i < n; i++) {
//             float mx = max_values[i];
